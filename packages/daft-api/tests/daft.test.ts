...
describe("DaftApi", () => {
  ...
  describe("sendMessage", () => {
    ...
    it("remints and retries on HTTP 403 when auto-minting", async () => {
      const prevRetries = process.env.DAFT_RECAPTCHA_SEND_RETRIES;
      const prevPrefer = process.env.DAFT_RECAPTCHA_PREFER_SHORT;
      process.env.DAFT_RECAPTCHA_SEND_RETRIES = "3";
      process.env.DAFT_RECAPTCHA_PREFER_SHORT = "0";
      try {
        let mintN = 0;
        let replyN = 0;
        const localFetch = mock((url: string, options: RequestInit) => {
          if (String(url).includes("/old/v4/reply")) {
            replyN++;
            if (replyN === 1) {
              return Promise.resolve(
        ...
        const client = new DaftApi({
          fetchFn: localFetch as unknown as typeof fetch,
          platform: "android",
          appVersion: "9.8.1",
          mintRecaptchaToken: async () => {
            mintN++;
            return {
              token: `mock-token-${mintN}-xxxxxxxx`,
              action: "enquiry_form_submit",
            };
          },
        });
        ...
        expect(mintN).toBe(2);
        expect(replyN).toBe(2);
        const [, options] = lastCall(localFetch);
        expect(options.headers).toMatchObject({
          "Recaptcha-Token": "mock-token-2-xxxxxxxx",
        });
        localFetch.mockRestore();
      } finally {
        if (prevRetries === undefined)
          delete process.env.DAFT_RECAPTCHA_SEND_RETRIES;
        else process.env.DAFT_RECAPTCHA_SEND_RETRIES = prevRetries;
        if (prevPrefer === undefined)
          delete process.env.DAFT_RECAPTCHA_PREFER_SHORT;
        else process.env.DAFT_RECAPTCHA_PREFER_SHORT = prevPrefer;
      }
    });

    it("prefers a short captcha token when minting", async () => {
      const prevPrefer = process.env.DAFT_RECAPTCHA_PREFER_SHORT;
      const prevTries = process.env.DAFT_RECAPTCHA_MINT_TRIES;
      process.env.DAFT_RECAPTCHA_PREFER_SHORT = "1";
      process.env.DAFT_RECAPTCHA_MINT_TRIES = "5";
      try {
        let mintN = 0;
        const client = new DaftApi({
          fetchFn: fetchFn as unknown as typeof fetch,
        ...
        expect(mintN).toBe(2);
        const [, options] = lastCall(fetchFn);
        expect(options.headers["Recaptcha-Token"]).toBe("S".repeat(100));
      } finally {
        if (prevPrefer === undefined)
          delete process.env.DAFT_RECAPTCHA_PREFER_SHORT;
        else process.env.DAFT_RECAPTCHA_PREFER_SHORT = prevPrefer;
        if (prevTries === undefined) delete process.env.DAFT_RECAPTCHA_MINT_TRIES;
        else process.env.DAFT_RECAPTCHA_MINT_TRIES = prevTries;
      }
    });
  });
